...
from .views import update_order_pricing
from django.contrib.auth import views as auth_views
from .views import google_one_tap_callback
...
path(
 ...
 path("admin/shipments/", views.shipment_dashboard, name="shipment_dashboard"),
    ...
    path('suppliers/verify-login/<uidb64>/<token>/', views.supplier_token_login, name='supplier_token_login'),
    path('suppliers/dashboard/', views.supplier_dashboard_view, name='supplier_dashboard'), # Protect this view w
    
    # 🌟 Secure routing path for container deep-dive administration panel
    path('shipments/<str:container_number>/', views.shipment_detail_view, name='shipment_detail'),
    path(
    "supplier-login/<uidb64>/<token>/",
    views.supplier_token_login,
    name="supplier_token_login"
),
path('submit_feedback/', views.submit_feedback_view, name='submit_feedback'),

path(
    "become-supplier/",
    views.become_supplier,
    name="become_supplier"
),
path("chatbot/", views.logistics_ai_chat, name="chatbot"),
path(
    "tracking/",
    views.tracking_search,
...
path(
    "supplier-payments/read/<int:pk>/",
    views.supplier_payment_read,
    name="supplier_payment_read"
),
...
path(
    ...
    views.google_one_tap_callback,
    name="google_one_tap_callback"
),
